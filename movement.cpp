 ...
 *  Collision Needs fixing; Collision was fixed it was just how collision was coded.
 * 
 * UPDATES:
...
struct Map : Items{
  ...
  bool objectCollision(Player *player) {
	...
	/*
	bool checkGrid(){
		if (dungeonMap[x][y] == selectedObj){
			std::cout << "The spot at (" << x << ", " << y << ") has the desired character: " << selectedObj << '\n';
            switch (selectedObj){
				case '|':
					std::cout << "You sit down at the table";
					break;
				case '@':
			...
			std::cout << "The spot at (" << x << ", " << y << ") does not have the desired character: " << selectedCharacter << '\n';
		}
